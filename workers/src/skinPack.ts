...
function faceRoleColor(
  ...
  switch (role) {
    ...
    case "chin_contour": return mixRgb(skinColor, shadeRgb(skinColor, 0.82), 0.46);
...
function applyFacePixelPlan(
  ...
  for (const pixel of plan.pixels) {
    ...
    if (pixel.cluster === "complexion" && pixel.role !== "cheek_contour" && pixel.role !== "jaw_contour" && pixel.role !== "chin_contour") continue;
