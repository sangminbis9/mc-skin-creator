...
export function buildFourViewPrompt(analysis: PhotoAnalysis): string {
  ...
  const lines = [
    ...
    `Face priorities: ${analysis.renderHints.faceShape} face, ${analysis.renderHints.eyeSize} ${analysis.renderHints.eyeShape} ${analysis.renderHints.eyeTilt} eyes, ${analysis.renderHints.eyeSpacing} spacing, ${analysis.renderHints.eyebrowShape} eyebrows, ${analysis.renderHints.noseShape} nose and ${analysis.renderHints.mouthShape} mouth with ${analysis.renderHints.lipFullness} lips.`,
    `Hair priorities: ${analysis.renderHints.bangsDensity} ${analysis.renderHints.bangsLength} ${analysis.renderHints.bangs} bangs with ${analysis.renderHints.fringeEdge} edge and ${analysis.renderHints.fringeOpening} opening; ${analysis.renderHints.hairTexture} ${analysis.renderHints.hairVolume}-volume ${analysis.renderHints.hairSilhouette} silhouette reaching ${analysis.renderHints.overallHairLength} length; ${analysis.renderHints.hairBackShape} back; ${analysis.renderHints.hairPart} part; ${analysis.renderHints.sideHairLength} ${analysis.renderHints.sideHairShape} side hair; ${analysis.renderHints.earExposure} ears.`,
    `Clothing priorities: ${analysis.renderHints.garmentTexture} texture, ${analysis.renderHints.outerGarment} outer garment, ${analysis.renderHints.neckAccessory} neck detail, ${analysis.renderHints.bottomPattern} bottom pattern, ${analysis.renderHints.bottomAccent} accent, ${analysis.renderHints.legwearAsymmetry} ${analysis.renderHints.legwear}, and ${analysis.renderHints.thighAccessory} thigh accessory on viewer-${analysis.renderHints.thighAccessorySide}.`,
    `Place the ${analysis.renderHints.hairAccessoryScale} ${analysis.renderHints.hairAccessoryColor} ${analysis.renderHints.hairAccessory} on the subject's correct ${analysis.renderHints.hairAccessorySide} side and show it only in views where that physical side is visible.`,
