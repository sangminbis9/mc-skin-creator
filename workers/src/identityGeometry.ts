...
export const IDENTITY_GEOMETRY_SCHEMA = {
  ...
  properties: {
    ...
    confidence: {
      type: "object",
      additionalProperties: false,
      properties: { faceBounds: unit, eyes: unit, brows: unit, nose: unit, mouth: unit, hairline: unit, glasses: unit },
      required: ["faceBounds", "eyes", "brows", "nose", "mouth", "hairline", "glasses"],
    },
  },
  required: ["face", "eyes", "brows", "nose", "mouth", "hairline", "glasses", "confidence"],
} as const;
...
FACE crop priorities: visible face bounds, forehead-to-chin span, each eye center/width/height, inter-eye distance, vertical asymmetry, brow height, nose contrast location, mouth center/width/corner offsets/opening and glasses footprint. HEAD crop priorities: face width within the full head silhouette, crown, hairline, part, fringe and head covering. hairline.depthByColumn has exactly eight left-to-right samples; each value is downward hair/fringe coverage from the visible forehead toward the eye line (0=no coverage, 1=reaches the eye line). Keep a smooth coherent profile; record real parts, widow peaks and asymmetric fringe, not photographic flyaway noise. Use null glasses only when no frame is visible. A weakly readable feature gets lower confidence rather than invented average geometry.`;
...
export function parseIdentityGeometry(raw: Record<string, unknown>): IdentityGeometryAnalysis | null {
  ...
  if (!face || !eyes || !brows || !nose || !mouth || !hairline || !confidence) return null;
  const unitValues = [
    ...
    confidence.faceBounds, confidence.eyes, confidence.brows, confidence.nose, confidence.mouth, confidence.hairline, confidence.glasses,
  ];
  ...
  return {
    ...
    glasses,
    confidence: { faceBounds: Number(confidence.faceBounds), eyes: Number(confidence.eyes), brows: Number(confidence.brows), nose: Number(confidence.nose), mouth: Number(confidence.mouth), hairline: Number(confidence.hairline), glasses: Number(confidence.glasses) },
    source: "normalized_face_head_crops",
  };
}
